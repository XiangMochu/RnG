...
class ProcessData(nn.Module):
    ...
    @torch.no_grad()
    def forward_single_target_view(self, input_batch, target_c2w):
        height, width = input_batch["image"].shape[3], input_batch["image"].shape[4]
        input_batch["image_h_w"] = (height, width)
        input_dict = edict(input_batch)

        target_batch = {"c2w": target_c2w, "fxfycxcy": input_batch["fxfycxcy"][:,:1], 
                        "image_h_w": (height, width)}
        target_dict = edict(target_batch)
        ...
        for dict in [input_dict, target_dict]:
            ...
            ray_o, ray_d = self.compute_rays(c2w, fxfycxcy, image_height, image_width, device=input_batch["image"].device)
            dict["ray_o"], dict["ray_d"] = ray_o, ray_d
        
        return input_dict, target_dict
    
    @torch.no_grad()
    def forward_target_view_without_input(self, target_c2w):
        ### this is hard-coded, improve later
        fxfycxcy = torch.tensor([[[351.6771096901917, 351.6771096901917, 128, 128]]], device=target_c2w.device)
        image_height = 256
        image_width = 256

        ray_o, ray_d = self.compute_rays(target_c2w, fxfycxcy, image_height, image_width, device=target_c2w.device)
